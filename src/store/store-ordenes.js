...
const actions = {
  async setFechaIni({ commit }, payload) {
    commit("FechaIni", payload);
  },
  async setFechaFin({ commit }, payload) {
    ...
    for (let index = 0; index < array.length; index++) {
      ...
      if (!element.enviado && element.billing.porcentaje >= 92) {
        ...
        const enviar = {
          name: element.billing.first_name,
          lastname: element.billing.last_name,
          comuna: element.billing.city_ori,
          direccion: element.billing.address_1,
          telf: element.billing.phone,
          tipodepago: element.tipodepago,
          control: "",
  ...
  async callOrdenes({ commit }, payload) {
    ...
    const response = await axiosInstance.get(
      `/ordenes/${payload.page}/${payload.cant}/${payload.ini}/${payload.fin}`
    );
    console.log("Todos los ordenes");
    commit("setOrdenes", response.data);
  ...
  async get_generarpdfs({ commit }, payload) {
    // console.log("Todos los GET mongolva");
    const response = await axiosInstancePdfs.post(
      `/fileserver/imprimirticketpdfarray/2`,
      payload
    );
